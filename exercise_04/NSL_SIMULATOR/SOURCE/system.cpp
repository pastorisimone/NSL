...
void System::step_back() { // Perform a simulation step backwards in time
    if (_sim_type == 0) this->Verlet();  // Perform a MD step
    else for (int i = 0; i < _npart; i++) this->move(int(_rnd.Rannyu() * _npart)); // Perform a MC step on a randomly choosen particle
    _nattempts += _npart; //update number of attempts performed on the system
    return;
...
void System::Verlet_back() {
    ...
    for (int i = 0; i < _npart; i++) { //Verlet integration scheme
        xnew = this->pbc(2.0 * _particle(i).getposition(0, false) - _particle(i).getposition(0, true) + _fx(i) * pow(_delta, 2), 0);
        ynew = this->pbc(2.0 * _particle(i).getposition(1, false) - _particle(i).getposition(1, true) + _fy(i) * pow(_delta, 2), 1);
        znew = this->pbc(2.0 * _particle(i).getposition(2, false) - _particle(i).getposition(2, true) + _fz(i) * pow(_delta, 2), 2);
        _particle(i).setvelocity(0, this->pbc(xnew - _particle(i).getposition(0, true), 0) / (2.0 * _delta));
        _particle(i).setvelocity(1, this->pbc(ynew - _particle(i).getposition(1, true), 1) / (2.0 * _delta));
        _particle(i).setvelocity(2, this->pbc(znew - _particle(i).getposition(2, true), 2) / (2.0 * _delta));
...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    cinf.close();
    cinf.open("../INPUT/CONFIG/vel_4_02.xyz");
    if (cinf.is_open()) {
        string comment;
        string particle;
        int ncoord;
        cinf >> ncoord;
        if (ncoord != _npart) {
            cerr << "PROBLEM: conflicting number of coordinates in input.dat & vel_4_02.xyz not match!" << endl;
            exit(EXIT_FAILURE);
        }
        cinf >> comment;
        double vx, vy, vz;
        for (int i = 0; i < _npart; i++) {
            cinf >> particle >> vx >> vy >> vz; //change time (sign)
            _particle(i).setvelocity(0, -vx);
            _particle(i).setvelocity(1, -vy);
            _particle(i).setvelocity(2, -vz);
        }
    }
    else cerr << "PROBLEM: Unable to open INPUT file vel_4_02.xyz" << endl;
    cinf.close();
  } else {
...
void System :: write_configuration(){
  ...
  if(_sim_type < 2){
    ...
    coutf.close();
    //ES 4.03:
    coutf.open("../INPUT/CONFIG/config_4_02.xyz");
    if (coutf.is_open()) {
        coutf << _npart << endl;
        coutf << "#Comment!" << endl;
        for (int i = 0; i < _npart; i++) {
    ...
    coutf.close();
    coutf.open("../INPUT/CONFIG/conf_4_02-1.xyz");
    if (coutf.is_open()) {
        coutf << _npart << endl;
        coutf << "#Comment!" << endl;
        for (int i = 0; i < _npart; i++) {
    ...
    coutf.close();
    coutf.open("../INPUT/CONFIG/vel_4_02.xyz");
    if (coutf.is_open()) {
        coutf << _npart << endl;
        coutf << "#Comment!" << endl;
        for (int i = 0; i < _npart; i++) {
    ...
    else cerr << "PROBLEM: Unable to open vel" << endl;
    coutf.close();
  } else {
