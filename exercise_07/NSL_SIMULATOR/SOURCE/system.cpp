...
void System::initialize_velocities_delta() {
    ...
    for (int i = 0; i < _npart; i++) {
        ...
        vy(i) = 0.;
        vz(i) = 0.;
        if (r < 1. / 3) vx(i) = sign;                      //         <|------------------<<<<<       fare con 6 e non con 3?
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = 8 * M_PI * _rho * (1. / (9. * pow(_r_cut, 9)) - (1. / (3. * (_r_cut, 3))));    // <|---------<<<<< profondità buca??
        ...
        index_property++;
        _ptail = 32 * M_PI * _rho * (1. / (9. * pow(_r_cut, 9)) - (1. / (6. * (_r_cut, 3))));    // <|---------<<<<< 
        ...
        _nprop++;
        _measure_chi = true;
        _measure_magnet = true;            //chi is given by <M^2>                              <|---------<<<<<<<
...
void System :: measure(){ // Measure properties
  ...
  vec distance;
  distance.resize(_ndim);
  double pofv_temp = 0.0;       //                                                      accu for p(v)          <|------------<<<<< fare array di zeros di dim n_bin_v?
  ...
  if (_measure_cv) {
    /*
      ...
      for (int i = 0; i < _npart; i++) {
          s_i = double(_particle(i).getspin());
          s_j = double(_particle(this->pbc(i + 1)).getspin());
          tenergy_temp += pow(-_J * s_i * s_j - 0.5 * _H * (s_i + s_j), 2);
      }
      tenergy_temp /= double(_npart);
      _measurement(_index_cv) = _beta * _beta * (tenergy_temp - _measurement(_index_tenergy) * _measurement(_index_tenergy))
    */
...
int System::equilibration(double  precision_or_steps, bool reset_accept) {   //argument: precision scale for invariant temperature (MD)
                                                                             //or number of steps to equilibrium (MC)
    int n_accepted = 0;                                                      //the function returns accepted moves before equilibrium, and
                                                                             //if reset_accept == True: resets _naccepted to 0
    
    //number of steps to reach equilibrium (MC)
    if (_sim_type == 0) {   //MD simulation
        if (precision_or_steps > 1) {
            cerr << "Wrong equilibration argument: input T precision for this type of simulation (MD)" << endl;
            exit(1);
        }

        bool oldflag = _measure_temp;
        _measure_temp = true;
        int contingency_steps = 1000000;            //to avoid an endless while loop
        int while_step = 0;
        double T_delta = 70007.;                    //Temperature difference for convergence, initial sentry value
        double old_T = _temp;
        double new_T = 0;
        while (T_delta > precision_or_steps) {
            this->step();
            this->measure();
            new_T += _measurement(_index_temp);

            while_step++;

            if (while_step % 100 == 0) {         //checks every 100 steps
                T_delta = abs(new_T/100 - old_T) / old_T;
                //cout << while_step << " " << new_T / 100 << " " << old_T << " " << T_delta << endl;
                old_T = new_T / 100;
                new_T = 0;
            }

            if (while_step > contingency_steps) {
                cerr << "equilibrium steps limit reached" << endl;
                break;
            }
        }
        //cout << endl << "steps to equilibrium: " << while_step << endl << endl << "T_delta: " << T_delta << endl << _temp << " " << old_T << " " << _measurement(_index_temp) << endl;

        this->block_reset(0);
        _measure_temp = oldflag;

        n_accepted = _naccepted;
        if (reset_accept == true) _naccepted = 0;
        return n_accepted;
    }
    else {  //MC simulations
        if (precision_or_steps < 1) {
            cerr << "Wrong equilibration argument: input number of steps for this type of simulation (MC)" << endl;
            exit(1);
        }
        int eq_steps = int(precision_or_steps);

        for (int i = 0; i < eq_steps; i++) this->step();
...
//compute instant U/N:
void System::output_U_inst() {

    ofstream coutf;

    // POTENTIAL ENERGY //////////////////////////////////////////////////////////
    if (_measure_penergy) {
        coutf.open("../OUTPUT/U_instant.dat", ios::app);
        coutf << setw(12) << _measurement(_index_penergy) << endl;
        coutf.close();
    }
}
void System::output_T_inst() {

    ofstream coutf;

    // TEMPERATURE ///////////////////////////////////////////////////////////////
    if (_measure_temp) {
        coutf.open("../OUTPUT/T_instant.dat", ios::app);
        coutf << setw(12) << _measurement(_index_temp) << endl;
        coutf.close();
    }
}

void System::inst_reset() {
    ofstream coutf;
    coutf.open("../OUTPUT/U_instant.dat");
    coutf.close();
    coutf.open("../OUTPUT/T_instant.dat", ios::app);
    coutf.close();
}
...
void System :: averages(int blk){
  ...
  if (_measure_gofr) {
      ...
      for (int i = 0; i < _n_bins; i++) {
          ...
          double norm = _rho * _npart * (4.0 / 3.0) * M_PI * (pow(r + _bin_size, 3) - pow(r, 3));
  ...
  if (_measure_pofv) {
      ...
      for (int i = 0; i < _n_bins_v; i++) {
          ...
          coutf << setw(12) << v
              << setw(12) << sum_average / double(blk)                                      //<|------------<<<<< aggiunto blk
  ...
  if (_measure_cv) {
      ...
      double H_ave = _average(_index_tenergy);
      double H2_ave = _average(_index_cv);
      average = _beta * _beta * (H2_ave - H_ave*H_ave) * double(_npart);
      double H_ave_glob = _global_av(_index_tenergy) / double(blk);
      double H2_ave_glob = _global_av(_index_cv) / double(blk);
      sum_average = _beta * _beta * (H2_ave_glob - H_ave_glob * H_ave_glob) * double(_npart);
      sum_ave2 = _global_av2(_index_magnet);        //     <|-----------<<<<<
      coutf << setw(12) << blk
          << setw(12) << average
          << setw(12) << sum_average / double(blk)
