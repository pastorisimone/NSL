...
private:
  ...
  // Properties
  int _nprop; // Number of properties being measured //-> 5 + bin
...
public: // Function declarations
  ...
  void initialize_velocities_delta();// Initialize particle velocities as a dirac delta                             <|-------------<<<<<<
  ...
  int equilibration(double  precision_or_steps, bool reset_accept = true);//equilibrate the system                  <|-------------<<<<<<
  void output_U_inst();       //output instant U/N                                                                  <|-------------<<<<<<
  void output_T_inst();       //output instant T                                                                  <|-------------<<<<<<
  void inst_reset();        //reset output file for instant U/N and T
  void averages(int blk);     // Compute averages of properties
  void averages_delta(int blk);// Compute averages of properties for a dirac delta                                  <|-------------<<<<<<
